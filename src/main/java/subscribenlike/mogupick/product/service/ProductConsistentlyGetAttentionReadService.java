...
public class ProductConsistentlyGetAttentionReadService {
    ...
    public void fetchConsistentlyGetAttentionProduct() {
        List<Product> products = productRepository.findAll();
        LocalDate recentlyOneYearAgo = LocalDate.now().minusYears(1);
        // TODO : 최근 12개월 이내 한 상품에 대하여 6개월 이상의 구독을 유지한 사람들 T, 그 외 F
        for(Product product : products) {
            List<Subscription> subscriptions = subscriptionRepository.findByProductId(product.getId());


            // TODO : T/(T+F) * 100의 값이 0.5 이상인 상품들의 리스트 구하기
        }
    }
